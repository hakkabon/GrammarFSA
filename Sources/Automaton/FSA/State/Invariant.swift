...
extension Deterministic {
    ...
    mutating func removeZombieAcceptStates() {
        guard case let .dfa(initial, finals, transitions, minimal, tokenMap) = self.state else { fatalError() }
    ...
    mutating func eliminateDeadStates() {
        guard case .dfa(let initial, var finals, var transitions, let minimal, var tokenMap) = self.state else { fatalError() }
        ...
        // Eliminate unreachable states and their transitions.
        var unreachable: [Int] = []
    ...
    mutating func removeDeadTransitions() {
        guard case .dfa(let initial, let finals, var transitions, let minimal, let tokenMap) = self.state else { fatalError() }
